...
def decompose_vagues(liste_niveaux):
    """
    Décompose une liste de niveaux en liste de vagues.
    
    Arguments :
    liste_niveaux -- Liste non vide de flottants représentant les niveaux de la mer
    
    Retourne :
    Liste de vagues (listes de niveaux)
    """
    vagues = []
    successeurs = construction_successeurs(liste_niveaux)
    
    for i in range(len(successeurs) - 1):
        vagues.append(liste_niveaux[successeurs[i]:successeurs[i + 1] + 1])
    
    return vagues

# Exemple d'utilisation
niveaux = [1, -1, -2, 2, -2, -1, 6, 4, -2, -5]
print(decompose_vagues(niveaux))  # Output: [[-1, -2, 2], [-2, -1, 6, 4]]